...
test("isTransaction() returns false for transaction with invalid signatureMessageFragment.", t => {
  t.is(
    isTransaction({
      ...transactionObject,
      signatureMessageFragment: transactionObject.signatureMessageFragment.slice(
        10
      )
    }),
    false,
...
test("isTransaction() returns true for valid transaction.", t => {
  t.is(
    isTransaction(transactionObject),
    true,
    "isTransaction() should return false for valid transaction."
  );
});
