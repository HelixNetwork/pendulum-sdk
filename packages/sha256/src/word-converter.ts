...
export function tritsToWords(trits: Int8Array): Uint32Array {
  if (trits.length !== 243) {
    throw new Error("Invalid trits length" + trits);
  }
...
export function bytesToWords(bytes: Int8Array): Uint32Array {
  if (bytes.length % 4 !== 0) {
 ...
 */
export function wordsToBytes(words: Uint32Array): Int8Array {
  let size = words.length;
  const bytes = new Int8Array(size * 4);
