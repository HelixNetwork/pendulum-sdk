...
export const createGetInputs = (provider: Provider) => {
  ...
  return (
    ...
    return Promise.resolve(
      validateGetInputsOptions(seed, { start, end, security, threshold })
    )
      ...
      .then(allAddresses =>
        getBalances(allAddresses, 100)
          .then(({ balances }) =>
            createInputsObject(allAddresses, balances, start, security)
          )
...
export const inputsToAddressOptions = ({
  start,
  end,
  security
        ...
        (acc: Inputs, input: Address) =>
          acc.totalBalance < threshold
            ? {
                ...
                totalBalance: acc.totalBalance + input.balance
              }
            : acc,
