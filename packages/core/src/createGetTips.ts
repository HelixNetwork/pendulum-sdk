...
import { Callback, GetTipsCommand, GetTipsResponse, Hash, ProtocolCommand, Provider } from '../../types'
...
export const createGetTips = ({ send }: Provider) =>
    ...
    (callback?: Callback<ReadonlyArray<Hash>>): Promise<ReadonlyArray<Hash>> =>
        send<GetTipsCommand, GetTipsResponse>({ command: ProtocolCommand.GET_TIPS })
            .then(({ hashes }) => hashes)
            .asCallback(callback)
