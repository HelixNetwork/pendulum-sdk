...
test("getBalances() resolves to correct balances response", async t => {
  t.deepEqual(
    await getBalances(
      getBalancesCommand.addresses,
      getBalancesCommand.threshold
    ),
    balancesResponse,
...
test("getBalances() rejects with correct errors for invalid input", t => {
  ...
  t.is(
    t.throws(
      () => getBalances(invalidAddresses, getBalancesCommand.threshold),
      Error
    ).message,
  ...
  t.is(
    t.throws(() => getBalances(getBalancesCommand.addresses, 101), Error)
      .message,
