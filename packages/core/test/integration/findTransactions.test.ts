...
const addressesWithChecksum =
  findTransactionsByAddressesCommand.addresses ||
  [].map((address: string) => address.concat("0".repeat(9)));

test("findTransactions() by addresses resolves to correct response", async t => {
  t.deepEqual(
    await findTransactions({
      addresses: findTransactionsByAddressesCommand.addresses
    }),
...
test("findTransactions() rejects with correct error for invalid addresses", async t => {
  t.is(
    t.throws(() => findTransactions({ addresses: invalidHashes }), Error)
      .message,
...
test("findTransactions() by bundle hashes resolves to correct response", async t => {
  t.deepEqual(
    await findTransactions({
      bundles: findTransactionsByBundlesCommand.bundles
    }),
...
test("findTransactions() by approvees resolves to correct response", async t => {
  t.deepEqual(
    await findTransactions({
      approvees: findTransactionsByApproveesCommand.approvees
    }),
...
test("findTransactions() rejects with correct error for invalid approvees", async t => {
  t.is(
    t.throws(() => findTransactions({ approvees: invalidHashes }), Error)
      .message,
