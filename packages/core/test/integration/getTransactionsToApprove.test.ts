...
const getTransactionsToApprove = createGetTransactionsToApprove(
  createHttpClient()
);

const addresses = ["a".repeat(2 * 32), "b".repeat(2 * 32)];
...
test("getTransactionsToApprove() with reference option resolves to correct response", async t => {
  t.deepEqual(
    ...
    {
      trunkTransaction:
        getTransactionsToApproveWithReferenceResponse.trunkTransaction,
      branchTransaction:
        getTransactionsToApproveWithReferenceResponse.branchTransaction
    },
    ...
    t.throws(
      () =>
        getTransactionsToApprove(
          getTransactionsToApproveCommand.depth,
          invalidHash
        ),
      Error
    ).message,
...
test.cb(
  "getTransactionsToApprove() passes correct arguments to callback",
  t => {
    getTransactionsToApprove(
      getTransactionsToApproveCommand.depth,
      undefined,
      (err, res) => {
        ...
        t.deepEqual(
          res,
          {
            trunkTransaction: getTransactionsToApproveResponse.trunkTransaction,
            branchTransaction:
              getTransactionsToApproveResponse.branchTransaction
          },
