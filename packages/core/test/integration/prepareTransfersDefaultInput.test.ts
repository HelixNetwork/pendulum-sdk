...
test("prepareTransfers() with network prepares the correct array of tx.", async t => {
  const hbytes = await prepareTransfersWithNetwork("abcd", transfers);
  t.deepEqual(
    hbytes,
    expected,
    "prepareTranfers() should prepare the correct array of tx."
  );
});
test("prepareTransfer() prepares correct tx for zero value transfers", async t => {
  ...
  t.deepEqual(
    ...
    "prepareTransfers() should prepare the correct tx for zero value transfers"
  );
});
